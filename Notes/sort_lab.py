...
def insertion_sort(my_list, index=0):
    for key_pos in range(1, len(my_list)):
        list_val = my_list[key_pos]
        key_val = my_list[key_pos][index]
        scan_pos = key_pos - 1  # look to dancer on the left
        while (scan_pos >=0) and (my_list[scan_pos][index] > key_val):
            my_list[scan_pos + 1] = my_list[scan_pos]
            scan_pos -= 1

        my_list[scan_pos + 1] = list_val

def selection_sort(my_list, index):
    for cur_pos in range(len(my_list)):
        min_pos = cur_pos
        for scan_pos in range(cur_pos + 1, len(my_list)):
            if my_list[scan_pos][index] < my_list[min_pos][index]:
                min_pos = scan_pos
        my_list[min_pos], my_list[cur_pos] = my_list[cur_pos], my_list[min_pos]  # commit the change

def reverse_list(my_list):
    for i in range(len(my_list) // 2):
        my_list[i], my_list[-1 - i] = my_list[-1 - i], my_list[i]

#insertion_sort(nba_leader_list, 1)
selection_sort(nba_leader_list, 1)
reverse_list(nba_leader_list)
